          ...
          <div className="flex items-center gap-4">
            {user ? (
              ...
              <div className="flex gap-3">
                ...
                <Link href="/signup" className="text-sm bg-secondary hover:bg-primary text-white px-4 py-2 rounded-lg transition-colors">
                  Sign Up
                </Link>
              </div>
            )}
        ...
        <div className="max-w-4xl mx-auto flex justify-around py-2">
          ...
          {user && (
            <Link
              href={`/profile/${user.id}`}
              className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg ${
                pathname.startsWith('/profile') ? 'text-accent' : 'text-slate-400'
              }`}
            >
              <UserIcon size={20} />
